% Exercise 5.4.2.e. Sketching Magnitude Responses from Pole/Zero Plots.
%	Pole & zeros & frequency response of the H5(z) digital filter - animation
%
...
%% poles & zeros
pp = [ 0.6086*exp(1i*2.17235)  0.6086*exp(-1i*2.17235) ].';
zz  = [ exp(1i*0.6972)  exp(-1i*0.6972) ].';
[b,a] = zp2tf(zz,pp,1);

figure('Name','Exercise 5.4.2.e. Sketching Magnitude Response from Pole/Zero Plots');
subplot(1,2,1);
zplane(b,a);
title(['                    (1 - e^{j0.6972}z^{-1})(1 - e^{-j0.6972}z^{-1})       ';
         'H_5(z)  = ----------------------------------------------------------------';        
         '             (1 - 0.6086e^{j2.1723}z^{-1})(1 - 0.6086e^{-j2.1723}z^{-1})  '
         '                                                                          ';
         '                                                                          ';]);
hold on;
ylim([-1.5 1.5]);
xlim([-1.5 1.5]);
disp('Exercise 5.4.1.e: pole/zero plot of H5(z) - Hit any key to animate')
pause
