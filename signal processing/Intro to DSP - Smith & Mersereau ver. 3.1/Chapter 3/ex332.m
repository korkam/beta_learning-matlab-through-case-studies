% Exercise 3.3.2. A Cascaded System.
...
[w H1] = my_DTFT(h1,n1); %#ok<*ASGLU>

[w H2] = my_DTFT(h2,n2);

figure('Name','Exercise 3.3.2. A Cascaded System');
subplot(3,2,1);
plot(w,abs(H1));
...
%% Step (b). Display the Magnitude of H3(jù) = DTFT{conv(h1[n],h2[n])}.

[n3 h3] = my_conv(h1,h2,n1,n2); % This signal has length of 7+6-1 = 12.
                                                        % Durates from n = 0 to n = 11. 
[w H3] = my_DTFT(h3,n3);
...
xlim([0 pi]);
grid on;
title('{|\itH_3}(j\omega)| = |{\itH_1}(j\omega)|*|{\itH_2}(j\omega)|');
hold on;
